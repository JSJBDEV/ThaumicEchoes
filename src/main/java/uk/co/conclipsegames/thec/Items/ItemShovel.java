        ...
        if(player.isSneaking())
        {
            ...
            while(z != aoe)
            {
                while(y != aoe)
                {
                    while(x != aoe)
                    {
                        if(world.getBlockState(new BlockPos(block.getX() +x, block.getY() +y, block.getZ() +z)).getBlock()== Blocks.DIRT || world.getBlockState(new BlockPos(block.getX() +x, block.getY() +y, block.getZ() +z)).getBlock()== Blocks.GRASS)
                        {
                            world.destroyBlock(new BlockPos(block.getX() +x, block.getY() +y, block.getZ() +z), false);
                            thePlayer.inventory.addItemStackToInventory(new ItemStack(Blocks.DIRT));
                        }
                        x++;
